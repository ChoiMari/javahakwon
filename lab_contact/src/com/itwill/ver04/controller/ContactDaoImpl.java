...
//import static 문장 : 클래스에서 public static으로 선언된 멤버(필드, 메서드)의 이름을 가져오기
// import static com.itwill.ver04.util.FileUtil.initializeData;
import static com.itwill.ver04.util.FileUtil.*; //* 가 모든 이라는 뜻. 모든 static 멤버 다 가져 오겠다.
...
public class ContactDaoImpl implements ContactDao {
	...
	private ContactDaoImpl() {
		//데이터 폴더 초기화 : 폴더가 없으면 새로 만듬.
		initializeDataDir(); //FileUtil.initializeDataDir();
		// 연락처 데이터 초기화 : 데이터 파일에서 리스트를 읽어오거나 , 빈 리스트를 생성
		contacts = initializeData(); //FileUtil.initializeData();
	}
	...
	@Override
	public int create(com.itwill.ver01.Contact contact) {
	...
	public int update(int index,Contact contact) {
		if(!isValidIndex(index)) { //유효한 인덱스가 아니면 실행
			return 0;
		}
