...
public class InnerMain03 {
	...
	public static void main(String[] args) {
		...
		//생성된 Button 객체가 할 일 정의 -> 리스너 객체를 생성하고 버튼에 세팅
...
//		btnClose.setOnClickListener(closeListener);	
		...
		//----- 익명 클래스를 사용한 버튼 객체 생성 및 리스너 설정1
		// 1. 버튼 생성
		Button btnAdd = new Button("Add");
		
		// 2. 리스너 구현 객체 생성
		OnClickListener addListener = new OnClickListener() { //생성자 호출이 끝이 아니라 클래스 바디까지
		...
		}; //; 생략 불가 선언된 클래스로 객체 생성하고 객체에다 저장하겠다 까지가 문장 ;생략시 컴파일 에러남
		// 지역클래스인데 객체 생성과 클래스 선언과 동시에 하겠다 -> 익명 클래스
