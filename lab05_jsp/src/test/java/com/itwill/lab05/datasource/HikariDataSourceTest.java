package com.itwill.lab05.datasource;

import java.sql.Connection;
import java.sql.SQLException;

import org.junit.jupiter.api.Test;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;

import com.zaxxer.hikari.HikariDataSource;

public class HikariDataSourceTest {
	
	private static final Logger log = LoggerFactory.getLogger(HikariDataSourceTest.class); //아규먼트로 클래스이름.class
	
	private final DataSourceUtil util = DataSourceUtil.getInstance();
	
	@Test
	public void test() throws SQLException {
		HikariDataSource ds = util.getDataSource();
				
		//System.out.println("test...");
		log.debug("ds = {}" , ds); //System.out.printf("ds = %s" , ds);와 비슷 하다고 생각하면 된다
		//{}안에는 값이 들어갈 자리. 타입은 상관 없다고..
		
		Connection conn = ds.getConnection(); //커넥션 풀에서 커넥션 객체를 빌려옴
		log.debug("conn = {}", conn);
		
		conn.close();//사용했던 커넥션(연결)을 커넥션 풀에 반환.
		log.debug("CP closed");
		
		
		
		
	}

}
