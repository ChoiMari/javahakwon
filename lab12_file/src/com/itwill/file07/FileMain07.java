...
//		
//		// *Java 11 이전 버전에서 문자 스트림과 인코딩을 다루는 방법
//		// 이전 버전에서는 FileReader,FileWriter 클래스 생성자에서 인코딩 타입을 설정할 수 있는 방법이 없음. 
//		
//		// 1. FileInputStream(바이트 스트림)객체를 생성
//		FileInputStream in = new FileInputStream(ansiFile);
//		
//		// 2. 인코딩을 설정한 InputStreamReader(문자 스트림) 객체를 생성 - 과거 방법
//		InputStreamReader isr= new InputStreamReader(in, "EUC-KR");
//		
//		// 3.(선택 사항.옵션) 읽기 속도를 빠르게 하기 위해서 버퍼(메모리 RAM)를 사용.
//		BufferedReader reader = new BufferedReader(isr); // 준비 끝
//		
//		while(true) { //무한 반복문
//			int read = reader.read(); //파일에서 한 글자 씩 읽어 들임
//			if(read == -1) { // EOF 파일 끝에 도달함
//				break;
//			}
//			System.out.print((char) read);
//			
//			reader.close();//리소스 해제(맨 마지막 생성 객체 close하면 아규먼트로 사용한 객체도 다 close된다고 함)
//		}
//			// *Java 11 버전부터 FileReader,FileWriter 클래스에서 인코딩을 설정할 수 있는 
//			// 생성자를 제공하기 시작.
//			
