...
---* RR과 YY 차이
--RR은 앞에 천자리를 자동계산 : 사람들이 생각하는 방식으로
--YY는 무조건 지금 기준
--지금 2천년대
--RR -> 2024 
--YY -> 2024
--그런데 년도가 99일 경우
--to_date('99/01/01', 'RR/MM/DD') -> RR은 자동계산 1999년
--to_date('99/01/01', 'YY/MM/DD') -> YY은 자동계산 2099년
...
-- (2) from에서 서브쿼리. ()출력 결과 테이블 그 테이블을 t라고 별명을 줌. 그 테이블에서 평균(별명 AVG_SAL)의 최댓값을 select 추출(검색)
...
select to_char(e.hire_date,'YYYY') HIRE_DATE, e.last_name
    from employees e
    where to_char(e.hire_date,'YYYY') = '2008';
    
-- 7. 2008년에 입사한 직원들의 부서 이름과 부서별 인원수 검색.
--표준 문법
select to_char(e.hire_date,'YYYY') HIRE_DATE , d.department_name, count(e.department_id) COUNT
    from employees e join departments d
    on e.department_id = d.department_id 
    where to_char(e.hire_date,'YYYY') = '2008'
    group by to_char(e.hire_date,'YYYY'), d.department_name, e.department_id;

--오라클 문법
select to_char(e.hire_date,'YYYY') HIRE_DATE , d.department_name, count(e.department_id) COUNT
    from employees e, departments d
    where e.department_id = d.department_id and to_char(e.hire_date,'YYYY') = '2008'
    group by to_char(e.hire_date,'YYYY'), d.department_name, e.department_id;
...
-- 9. 부서번호, 부서별 급여 평균을 검색. 소숫점 한자리까지 반올림 출력.
--표준 문법
select e.department_id, round(avg(e.salary),1) AVG_SALARY
    from employees e 
    join departments d 
    on e.department_id = d.department_id 
...
select e.department_id, round(avg(e.salary),1) AVG_SALARY
    ...
    on e.department_id = d.department_id 
    group by e.department_id
    having avg(e.salary) = (select max(avg(e.salary)) from employees e, departments d group by e.department_id);
...
-- 12. 국가이름, 국가별 급여 합계 검색
--표준 문법
select c.country_name, sum(e.salary) SUM_SAL
    from employees e 
    join departments d on e.department_id = d.department_id
    join locations l on d.location_id = l.location_id 
    join countries c on l.country_id = c.country_id
...
-- 14. 직무 이름, 직무별 급여 평균, 최솟값, 최댓값 검색.
--표준 문법
select j.job_title, avg(e.salary) AVG_SAL, min(e.salary) MIN_SAL, max(e.salary) MAX_SAL
    from employees e 
    join jobs j
    on e.job_id = j.job_id
    group by j.job_title;
...
-- 15. 국가 이름, 직무 이름, 국가별 직무별 급여 평균 검색.
--표준 문법
select c.country_name, j.job_title, avg(e.salary) AVG_SAL
    from employees e 
    join jobs j on e.job_id = j.job_id
    join departments d on  e.department_id = d.department_id
    join locations l on d.location_id = l.location_id
...
select c.country_name, j.job_title, avg(e.salary) AVG_SAL
    ...
    where e.job_id = j.job_id and e.department_id = d.department_id and d.location_id = l.location_id and l.country_id = c.country_id
    group by c.country_name,j.job_title;
    
-- 16. 국가 이름, 직무 이름, 국가별 직무별 급여 합계을 출력.
--     미국에서, 국가별 직무별 급여 합계가 50,000 이상인 레코드만 출력.
--표준 문법
select c.country_name, j.job_title, sum(e.salary) SUM_SAL
    from employees e 
    join jobs j on e.job_id = j.job_id
    join departments d on  e.department_id = d.department_id
    join locations l on d.location_id = l.location_id
...
select c.country_name, j.job_title, sum(e.salary) SUM_SAL
    ...
    where e.job_id = j.job_id and e.department_id = d.department_id and d.location_id = l.location_id and l.country_id = c.country_id
    and c.region_id = r.region_id and r.region_name = 'Americas'  
    group by c.country_name,j.job_title
    having sum(e.salary) >= 50000;
